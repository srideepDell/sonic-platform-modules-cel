/*
*   cpld_2.c -> CPLD_2 driver for Midstone-100TH2.
*
*
*
...
/*
 * CPLD2 management QSFP Ports 17 to 32
 * Device address : 0x31
 */
...
static ssize_t cpld2_dump_show(struct device *dev, struct device_attribute *attr, char *buf)
{
  ...
  for (index = 0 ; index < fpga_data->cpld2_read_length ; index++)
  {
    fpga_i2c_access(fpga_data->i2c_adapter[VIRTUAL_I2C_CPLD_INDEX], CPLD2_SLAVE_ADDR, 0x00,
                    I2C_SMBUS_READ, fpga_data->cpld2_read_addr + index, I2C_SMBUS_BYTE_DATA,
                    (union i2c_smbus_data*)&data[index]);
  }

  memcpy(buf, data, fpga_data->cpld2_read_length);
  
  return fpga_data->cpld2_read_length;
}
...
static ssize_t cpld2_setreg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
  ...
  err = fpga_i2c_access(fpga_data->i2c_adapter[VIRTUAL_I2C_CPLD_INDEX], CPLD2_SLAVE_ADDR, 0x00, I2C_SMBUS_WRITE, addr, I2C_SMBUS_BYTE_DATA, (union i2c_smbus_data*)&data);
  if(err < 0)
...
/* cpld2 scratch register for test reading/writing
 * Operation : R/W
 * data      : data which want to write into register
 * Usage
 * $ echo data > scratch
...
static ssize_t cpld2_version_show(struct device *dev, struct device_attribute *attr, char *buf)
{
  ...
  err = fpga_i2c_access(fpga_data->i2c_adapter[VIRTUAL_I2C_CPLD_INDEX], CPLD2_SLAVE_ADDR, 0x00,
                        I2C_SMBUS_READ, 0x00, I2C_SMBUS_BYTE_DATA,
                        (union i2c_smbus_data*)&data);

  return sprintf(buf, "%#x\n", data);
}
struct device_attribute dev_attr_cpld2_version = __ATTR(version, 0600, cpld2_version_show, NULL);

/* Software reset of Switch CPLD2
 * Operation : R/W
 ...
 * $ cat reset
 */
static ssize_t cpld2_reset_show(struct device *dev, struct device_attribute *attr, char *buf)
{
  int err;
  uint32_t data;
...
static ssize_t cpld2_reset_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
  int err;
  uint8_t data;
  uint32_t buffer;
...
struct device_attribute dev_attr_cpld2_reset = __ATTR(reset, 0600, cpld2_reset_show, cpld2_reset_store);
...
static struct attribute *cpld2_attrs[] = {
  ...
  &dev_attr_cpld2_reset.attr,
  NULL,
};

struct attribute_group cpld2_attr_grp = {
  .attrs = cpld2_attrs,
};
